...
function SetExistingTasks(taskPriority, taskName) {
  ...
  var html = "<div id=\"".concat(taskCount, "-obj\" class=\"todo-object\"><a class=\"todo-priority ").concat(taskClass, "\">").concat(taskPriority, "</a><h3 class=\"todo-title\">").concat(taskName, "</h3><button onclick=\"RemoveTask(").concat(taskCount, ", '").concat(taskName, "', '").concat(taskPriority, "')\" id=\"finished-button-").concat(taskCount, "\" class=\"stylized-button todo-button\">Finish</button></div>");
...
function ReadCookies() {
  if (localStorage.getItem("userData") === null) {
    ...
    var data = localStorage.getItem("userData");
    var _userData = JSON.parse(data);
    for (var i = 0; i < _userData.tasks.length; i++) {
      taskName = _userData.tasks[i].name;
      taskPriority = _userData.tasks[i].priority;
